...
for hh=1:numel(Delta_S_array)
    for hhh=1:numel(Delta_D_array)
        Delta_S0 = 2*pi*Delta_S_array(hh);
        Delta_S1 = 2*pi*Delta_S_array(hh);
        Delta_D1 = 2*pi*Delta_D_array(hhh);
        Delta_D2 = 2*pi*Delta_D_array(hhh);
        Ba133_SPD_Levels_OpticalBloch_Sim_fast
        sigma_PP_end(hh,hhh) = real(sigma_t(P0P0,end)+sigma_t(P1P1,end));
    end
end