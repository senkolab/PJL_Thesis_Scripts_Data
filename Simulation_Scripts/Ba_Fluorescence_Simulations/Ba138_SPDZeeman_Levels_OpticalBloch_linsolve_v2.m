...
B = 1*835*10^(-6); %Magnetic field strength in Gauss.
...
pol_minus_s = sqrt((cos(theta_s)^2)/2); pol_0_s = sqrt(sin(theta_s)^2); pol_plus_s = sqrt((cos(theta_s)^2)/2); %S-P laser polarization components.
pol_minus_d = sqrt((cos(theta_d)^2)/2); pol_0_d = sqrt(sin(theta_d)^2); pol_plus_d = sqrt((cos(theta_d)^2)/2); %D-P laser polarizatoin components.
